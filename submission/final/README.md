...
> Stand: 2026-09-08 (v0.13.2) | Phase-3b-Anhangnotiz 2026-09-16 (committed 2026-09-16): Appendix W (`docs/paper/08_appendixW_weight_vs_human.md`) ist exploratory appendix-only und NICHT im assemblierten PDF (`scripts/build_paper_pdf.py` ORDER = Kernkapitel only); Design-vs-Execution-Richtertrennung siehe `declaration_of_support.md` §1 + Paper §2.12; Zahlenstand unten unverändert (177 Tests = 59×3 verifiziert, übernommen — s. `plattform_antworten.md` §6; file-truth 62/57/186).
...
| F12 | LLM-within-subject Signal +0,08–0,09 (59 Messungen, 54 Modelle; file truth 62/57/186 incl. qwen-max n=26/30) | ~81 % CN-Anbieter; 9/177 EN-Paare n. s.; 177 Tests = 59×3 (file-truth 186 = 62×3) |
...
- Guard 2026-09-16 verifiziert: `manifest.json` = 556 / 517 / 219 (graph.total_nodes / alignment.total_relations / alignment.aligned_groups); pytest 84 grün; `docs/paper` arXiv-Marker = 18.
- Physik 367 / Chemie 220 (Legacy-Pipelines) → gesamt 1.140+ Konzepte
- Gold: 92 (sozial F1 0,939† Developing C9b n=72, harness ~0,65, blind pending; gewichtet gesamt 0,881)
- Human: N=15 (Between, Δ≈0) + LLM-within (Signal)
- 59 Messungen: 42 DashScope + 8 zen/OpenRouter + 2 Kilo + 1 Cohere + 1 NIM + 1 Cloudflare + 1 LM-Studio + 3 opencode-go = 59 (54 Modelle); alle ZH-DE p<0,05 (p=0,0 als p<0,004 lesen)
- Westlich (11 Messungen, 10 Identitäten ohne luna): nemotron-3-ultra + nemotron-3-super (NVIDIA), laguna-s-2.1 (zen + Kilo, 2 Hosts), gpt-oss-20b (NIM), command-a (Cohere), llama-3.3-70b (Cloudflare), grok-4.6 + muse-spark (Terminal), phi-4-mini (LM-Studio, lokal), gpt-5.6-luna (Herkunft ungeklärt)
