...
def multi_data() -> dict:
    ...
    Caps the per-model bar display at the top-8 margins; reports the full
    model count so the layperson view stays readable at 50+ models."""
    d = load_latest("multi_model_replication_*.json", LLM_OUT)
    margins = d["comparison"]["per_pair"]["ZH-DE"]["margin"]
    n_total = len([v for v in margins.values() if v is not None])
    top = sorted([(m, round(v, 3)) for m, v in margins.items() if v is not None],
                 key=lambda x: -x[1])[:8]
