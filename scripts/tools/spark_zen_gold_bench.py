...
Usage: python scripts/tools/spark_zen_gold_bench.py [--model ID] [--transport zc-free|minimax|r4|sn] [--ns NS] [--timeout S] [--sleep F] [--max-items N] [--cell P0|P1|P2|P3] [--langs zh,de,en]
...
# Harness v2 cells (A12) — single source of truth: research/mimo_spark_replication/_v2_cells.json
# (generated by stage0_v2_cells.py from frozen v1 strings; P0 == v1 byte-identical).
def _load_cells():
    import json as _j
    p = os.path.join(OUT_DIR, "_v2_cells.json")
    cells = _j.load(open(p, encoding="utf-8"))
    assert hashlib.sha256(cells["P0"]["system"].encode("utf-8")).hexdigest() == SYSTEM_SHA256
    assert hashlib.sha256(cells["P0"]["template"].encode("utf-8")).hexdigest() == TEMPLATE_SHA256
    return cells
...
def call(model, text, key, timeout, prompt_id="p", transport="zc-free"):
    sys_p, tpl_p = CELLS[ACTIVE_CELL]["system"], CELLS[ACTIVE_CELL]["template"]
    user = tpl_p.format(text=text[:2000])
    if transport == "minimax":
        return call_minimax(model, sys_p, user, timeout, 0.1), "system-param"
    if transport == "r4":
        return call_r4(model, sys_p, user, timeout, 0.1)
    if transport == "sn":
        return call_sn(model, sys_p, user, timeout, 0.1)
    composed = "[SYSTEM-ROLE]\n" + sys_p + "\n\n[TASK]\n" + user
...
def run_all(model, timeout, sleep_s, transport, ns, max_items=0):
    ...
    def build_out(recs, complete):
        ...
        return {"model": model, "ns": ns,
                ...
                "system_sha256": hashlib.sha256(CELLS[ACTIVE_CELL]["system"].encode("utf-8")).hexdigest(),
                "template_sha256": hashlib.sha256(CELLS[ACTIVE_CELL]["template"].encode("utf-8")).hexdigest(),
    ...
    recs = []
    for it in items:
        sid0 = it.get("sample_id", it.get("id", "unknown"))
        if sid0 in done:
            recs.append(done[sid0])
    for idx, item in enumerate(items):
        ...
        if LANGS_FILTER and item.get("language", "") not in LANGS_FILTER:
            continue
        if max_items and n_processed >= max_items:
        ...
        if len(tail) == 5 and not any(r.get("valid") for r in tail):
            ...
            return 6
        time.sleep(sleep_s)
    out = build_out(recs, not max_items and not LANGS_FILTER)
    json.dump(out, open(out_path, "w", encoding="utf-8"), ensure_ascii=False)
