...
> **更新**: v2（7 个完整模型，含美国来源 NVIDIA）——v1 为 4 模型

---

## 0. 一句话结论

**跨语言价值观分歧是"多语言 LLM 的普遍属性"，跨越中西方来源模型**：7 个模型（DeepSeek/智谱/Moonshot/字节/laguna/**NVIDIA 美国**）在完全相同协议下，**全部 21 个语言对 LDS-C ≫ 组内底噪、置换检验 p<0.05**；且文化方向（DE-自主/规则 vs ZH-关系/空间/应得）跨模型一致（116 概念 ≥3/7 一致，7 个概念 ≥6/7，3 个概念 7/7）。**"单模型 PoC"与"全是中国模型"两项限制同时关闭。**
...
- 所有 21 个语言对 LDS-C（0.84–0.96）显著高于各自 floor（0.74–0.87），p<0.05（多数 p=0.0）
- **跨中西方来源复现**：NVIDIA（美国）模型显示与 glm/kimi 同量级（甚至更高）的 ZH-DE 分歧 → 非中国模型的独立确认

## 3. 幅度因模型而异（审计工具的实用维度）

- glm/kimi/nemotron 余量 +0.135~0.155，约为 deepseek 族（+0.065~0.084）的 2 倍
- → 不同模型的"跨语言漂移敏感度"不同——审计报告可按漂移幅度给模型排名

## 4. 方向复制：116 概念跨 7 模型一致

**方法**：逐概念统计各模型把它标为 DE-only 还是 ZH-only；≥3/7 模型同侧 = 方向一致。（top-10 Jaccard 偏低是频率排序所致；方向一致性才是正确的稳健性度量。）
