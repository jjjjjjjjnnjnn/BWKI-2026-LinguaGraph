...
> （**主动诚实，避免被动**）"不是全部语言对。**55 个中德语言对全部显著**（p<0.05）；但 **165 个语言对中有 8 个不显著（p≥0.05），全部含英语**（ZH-EN 或 DE-EN），主要是 DeepSeek R1 蒸馏族——例如 deepseek-r1-0528 的 DE-EN p=0.672。我们的措辞已修正为'全部 ZH-DE 显著，8 个英语对不显著'。数据在 `multi_model_replication_20260910.json`，可复现。（计数口径：55 完整测量＋50 身份；qwen-max n=29 部分测量在 ZH-DE 同样显著，公开计数时除外，见 SSOT-web 双账。）"
...
> "方向信号是**统计显著的**（≥10 票 218 概念 vs 随机 147±4，p<0.001），但我们要诚实：(1) 分母是 Voteset n=56（55 测量 / 50 身份），Heimat:safety 只有部分模型产出——凡'全票'表述一律按 n=56 核验，不报全票；(2) physical space 有 1 个模型投了反方向；(3) 这是描述性方向，不是因果解释——需要文化心理学（如 Markus & Kitayama [37]）扎根。（定义：218＝达阈概念数 n_consistent_at_report_threshold；147.1±4.2＝permutation 零模型均值±SD；n=56 为 56 个成功模型投票；出处见 JSON direction_consistency 段。）"
...
> "能。所有结果由随代码提交的数据文件支撑：`data/lds_c/llm_subject/`（各模型 30 单元）+ `multi_model_replication_20260910.json`（55 测量对比）+ `design_effect_20260810.json`（设计效应）。脚本：`scripts/lds_c_multi_model.py` 等。`git clone` + 已提交数据可重跑。任何'全部/全票'表述我们已修正为可验证的诚实措辞。"
...
| 追问 | 对策 |
|:-----|:-----|
| "55/55 ZH-DE 全显著？打开 JSON" | 主动承认 8 个英语对不显著（8/165，不含 qwen-max 部分测量）；指向数据文件 |
