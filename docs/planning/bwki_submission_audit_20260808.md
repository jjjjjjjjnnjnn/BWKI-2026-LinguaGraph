...
**一句话**: "LinguaGraph 用量化方法回答：语言是否改变知识组织？——是的，但只有剥开制度知识的强制统一、并用受控被试设计（LLM-as-Subject）之后才可见。"

**三层展开**:
1. **原创方法**: LDS v3（图结构分歧）+ 组内设计（同一权重集三语）
2. **完整证据链**: 人类组间阴性（设计伪影）→ LLM 组内阳性（信号真实）→ 机制（代码层主导）→ 语料（制度抹平/文化暴露）
3. **方法学贡献**: 组间/组内检测力差异 + 人类组内设计蓝图
