...
> 更新日期: 2026-08-08 | 提交截止: **2026-09-20/21**（约 6 周）
...
## 0. 执行进度（2026-08-08 更新，含 A3/A5/D 深化）

> 完整交接见 [session_handoff_20260808.md](../session_handoff_20260808.md)（v0.9）。

| 任务 | 状态 | 结果摘要 |
|------|------|---------|
| **A0 治理+血缘** | 🔶 部分 | 未提交数据已 commit；数字口径（556/557、219/247）待对齐 |
| **A1 提取+建图** | ✅ 完成 | 15/15 概念 + 15/15 关系（deepseek-v4-flash） |
| **A2 LDS-C 计算** | ✅ 完成 | 概念级 + v3 节点+边；空模型确认无语言信号 |
| **A3 ΔLDS 核心分析** | ✅ 完成 | 人类组间 ΔLDS ≈ 0 → 设计伪影 |
| **A4 LDS-K 深化** | ✅ 完成 | Wikipedia 对齐修复 + 社会/制度模式相反 + 敏感性 + 跨源空模型（`lds_k_deepen.py`） |
| **A5 可解释性** | ✅ 完成 | ZH-DE 由框架负载概念驱动（DE 自主/规则 vs ZH 空间/应得）；关系驱动者（`lds_c_divergence_drivers.py`） |
| **A6 基线整合** | ⬜ 待做 | 论文 §8.8 已标注"方法上不再成立"；调和可标注历史 |
| **A7 核心图表** | ✅ 完成 | 5 图 + CSV（`fig_a7_core.py`，outputs/figures/） |
| **D1 招募** | 🔶 部分 | EN 3 条（双语试点）已纳入；目标 10 未达 |
| **D2 模型** | ✅ 完成 | deepseek-v4-flash @ opencode GO 验证（踩坑见交接 §5） |
| **D3 主题映射** | ✅ 完成 | 规范顺序 [Freiheit, Gerechtigkeit, Verantwortung, Heimat, Erfolg] 已验证 |
| **🆕 D1 机制实验** | ✅ 完成 | **LLM-as-Subject 组内设计：语言信号真实（LDS-C 0.93–0.96 ≫ 底 0.85–0.87）**；LMM 判定代码层主导（same_lang +0.038 p<0.001）、框架次级（p=0.90）；见 `docs/d1_mechanism_results.md` |
| **🆕 A3 设计效应证明** | ✅ 完成 | **人类/LLM 信号幅度相同（0.93–0.96），底噪不同（人类 0.92–0.96 vs LLM 0.85–0.87）**；非 N 效应（N=3 仍有信号）；虚拟组间 lens 证明=组内个体异质性；见 `docs/a3_c_d_deepen_results.md` |
| **🆕 A5 分歧驱动者** | ✅ 完成 | RQ4 已答：ZH-DE 由框架负载概念驱动（DE 自主/规则 vs ZH 空间/应得） |
| **🆕 D 节点/边分解** | ✅ 完成 | 反转是节点驱动（数学 node-only 0.444 vs 社会 0.800）；边贡献数学最大（0.074）——制度收敛于概念，关系仍分歧 |
| **🆕 论文整合** | ✅ 完成 | D1（§5）+ A4（§3.8）+ A3/A5/D（§5.9）+ 讨论（§8.14）+ 证伪框架（§6.2/6.3） |

**核心科研结论（完整证据链）**: ① 人类 N=15 组间三层阴性（设计伪影，信号幅度与 LLM 相同）；② **LLM-as-Subject 组内设计证明语言信号真实存在**；③ **设计效应证明：人类阴性 = 组内个体异质性，非"无效应"、非"组间设计本身"**；④ **LMM 机制判定：语言代码层主导（M2），文化框架次级（M3）**；⑤ **语料侧：社会 Wikipedia ZH-DE 0.82 vs 数学 0.52——制度知识抹平语言，文化概念暴露语言（节点驱动）**；⑥ **ZH-DE 分歧由框架负载概念驱动（RQ4）**。
...
| RQ | 研究问题 | 状态 |
|----|----------|------|
| RQ1 | LDS 能否量化跨语言知识图结构差异？（方法验证） | 部分完成（F1 0.939† Developing：ZH 0.974 / DE 0.949 / EN 0.882；19 模型 benchmark） |
...
| 风险 | 缓解 |
|------|------|
| 弱模型提取质量（0.5B GGUF） | D2 门控：F1 vs gold labels（0.939† Developing 目标，盲审后复核） |
