...
Die Robustheit dieses Befundes über drei Sprachen hinweg (ZH, EN, DE) deutet darauf hin, dass es sich nicht um ein Artefakt einer bestimmten Lehrbuchtradition handelt. Vielmehr könnte es ein Muster der untersuchten STEM-Lehrbuchgraphen (ZH/EN/DE, Preliminary) widerspiegeln — kein Universalitäts-Anspruch — oder zumindest eine Konvergenz über drei unterschiedliche Bildungssysteme hinweg.
...
Die theoretische Implikation ist deskriptiv: nominelle ZH-DE-Ähnlichkeit unter Structure-Null-Erwartung (frozen v3) — T1-falsifiziert als Label-/Größen-Artefakt; kein Stärkebeweis, keine Unabhängigkeits-Aussage über Bildungstraditionen. Drei unterschiedliche Bildungstraditionen (Chinesisch, Deutsch, Englisch) produzieren Lehrbuchwissensgraphen, deren strukturelle Eigenschaften (Gradverteilungen, Dichteprofile) einander deskriptiv ähnlicher sind als vergleichbare Graphen mit derselben Gradsequenz.

Dies bedeutet, dass der Korpusanalyse-Ansatz (LDS-K) per se keine sprachrelativistischen Effekte auf die Wissensorganisation messen kann. Er misst in erster Linie **strukturelle Ähnlichkeit**, die von geteilter Gradstruktur (Preliminary) getragen wird. Um einen kognitiven Anteil jenseits der Lehrbuchstruktur zu schätzen (ΔLDS, Ebenen-getrennt, Preliminary), müssen wir zur kognitiven Ebene übergehen — dem Vergleich, wie Menschen ihr Wissen in ihrer Muttersprache ausdrücken — erfasst durch ΔLDS = LDS-C − LDS-K.
...
Dieser Befund hat Implikationen für die Lehrplangestaltung — korrelativ, keine Kausalzuordnung (§8.12): Wenn Mathematik- und Physikstudierende grundlegend unterschiedliche Wissensdichtetrajektorien durchlaufen, dann sind pädagogische Strategien, die für eine Disziplin wirken, möglicherweise nicht auf die andere übertragbar. Der Mathematikunterricht könnte frühe Integration betonen; der Physikunterricht könnte akzeptieren, dass Integration auf fortgeschrittenem Niveau ein natürlicher Bestandteil des Lernverlaufs ist.
...
Der Coverage-Score misst die **Lehrplan-→Lehrbuch-**Übereinstimmung: Findet sich zu jedem Lehrplankonzept ein entsprechendes Konzept im Lehrbuchgraph? Die dramatische Spanne — von 12,7 % (NRW) bis 95,4 % (CN) — ist als Messung robust; ihre Deutung als Governance-Effekt (zentral vs. föderal) bleibt eine Hypothese (s. §8.6: Granularität CN 87 vs. US 2124 vs. NRW 299 als sicherste Lesart).
...
Ein potenzielles Bedenken bei jeder LLM-basierten Analyse ist, ob Messfehler die berichteten Ergebnisse verursachen könnten. Unsere Extraktionsvalidierung anhand von 92 Goldstandard-Annotationen (Batch A Mathe n=20 hand-annotiert; Batch B Sozial n=72 machine-seeded/human-accepted, **Developing (C9b)** — Sozial-Subgruppe ZH F1=0,974†, DE F1=0,949†, EN F1=0,882†; gewichtet gesamt 0,881) deutet darauf hin, dass die Extraktionsqualität des früheren primären Extraktionsmodells (qwen-plus) auf dem DB-Pfad vorläufig hoch, jedoch Developing (C9b) ist. **Limitation (Same-Source)**: Batch-B-Seeds und Evaluierungsmodell sind identisch (qwen-plus); der DB-Pfad-Sozial-F1 (0,939) enthält einen Verwandtschaftsbonus — unabhängig per Harness reberechnet liegt Sozial bei ~0,65 (s. G3-Notiz `research/gold_deconfound_2026-09-14.md`). Die Fehleranalyse zeigt, dass 29 % der Extraktionsfehler bei sehr kurzen Antworten (1–2 Wörter) auftreten, bei denen eine leere Extraktion tatsächlich angemessen ist. Bei den verbleibenden Fehlern handelt es sich überwiegend um partielle Auslassungen — 1–2 Konzepte aus einer Liste von 3–4 fehlen — und nicht um systematische Fehlleitungen.

**● Hinweis zur N=15-Erweiterung**: Die erweiterte Humanvalidierung (§4) wurde mit `deepseek-v4-flash` (opencode GO) extrahiert; eine erneute F1-Validierung gegen die Goldstandards steht aus. Die Within-Subject-Replikation (59 Messungen / 54 Modelle, §5.10, Verfügbarkeitsstichprobe, ohne Mehrfachtest-Korrektur) zeigt konsistente Skalen (alle 59 ZH-DE-Paare signifikant, p<0,05; 9/177 englisch-haltige Tests n.s.), sodass eine modellinduzierte Skalenverschiebung der LDS-C zwar möglich, eine systematische Verzerrung der Null-Relation (beobachtet ≈ Split-Half) aber unwahrscheinlich ist. (Der frühere 19-Modell-Extraktionsbenchmark ist als Vorstudie archiviert, s. §8.9 — kein Evidenz-Rang.)

Diese Fehlerverteilung bedeutet, dass die strukturellen Metriken (CDS, HDS, LDS, Coverage-Score) auf Topologie-Ebene robust gegenüber Extraktionsrauschen sind: Partielle Auslassungen reduzieren die Konzeptanzahlen leicht, verzerren jedoch nicht systematisch die Graphentopologie oder die sprachübergreifenden Vergleiche. Wir halten es daher für unwahrscheinlich, dass die berichteten *Topologie*-Befunde (CDS/HDS/LDS-Rangordnung) allein Artefakte der Extraktionsmethodik sind; für die Gold-Validierung gilt dies nicht (C9b Developing, ~15 % falsify-grade Lücke, s. TRACE §8.7).

> **TRACE Harness v2 / Panel (C21/C23/C24) + E2 (D-V11)**: Der seed-fremde Faktortest (§2.9b) bestätigt die Harness-v1-Konfundierung (**C23**: CARD-Fix P1−P0 +0,34/+0,41/+0,27 (glm-4.7-flash/deepseek-v4-flash/qwen3-6.8b; alle CI≠0; glm drift-konfundiert = obere Schranke, sauber deepseek/6,8); LANG-Fix ohne EN-Lift — P2-EN 0,02/0,14/0,00, alle <0,30; CJK→0 %; P2-Ankervergleich n-asymmetrisch (6,8 n=92 vs. 66), daher deskriptiv; EN-Lift aus Kardinalität, nicht Sprachklausel) und liefert die quantitative v2-partial-repro (**C21**: P3 sozial deepseek 0,544 validiert / glm-4.7-flash 0,544 drift-caveatiert / qwen3-6.8b 0,395 marginal miss ohne Aufrunden / big-pickle 0,580 deskriptiv); der Western-EN-Effekt bleibt Hypothese (**C24**, big-pickle deskriptiv). **E2**-Paraphrase (P3b, glm n=30): |P3b−P3| +0,024, CI [−0,0467–+0,0913] enthält 0 und überschreitet ±0,05 — Äquivalenz NICHT gezeigt, nur „kein signifikanter Unterschied" (explorativ; D-V11). Das Agent-Panel (falsify-only, 5 Endpunkte, MAINTAIN **weak, single-run, unkalibriert**: vs-gold 0,389, qwen-F1 0,419, κ=0,56<0,61, Jaccard 0,44–0,77; 15 Rejects total, davon 11/72 einstimmig ~15 % falsify-grade Gold-Lücke; kein Retest/Swap/Human-Baseline) — C9b bleibt Developing, die obige Robustheits-Einschätzung gilt nur für die Topologie-Ebene, nicht für die Gold-Validierung.
...
**Diese Vergleich ist mit der N=15-Erweiterung nicht mehr haltbar** (C7-Pilot → C17-Negativresultat), aus zwei Gründen:
...
### 8.9 Robustheitsprüfung: Modellübergreifende Extraktionskonsistenz — ARCHIVIERT (entfernte Vorstudie)

> **Status: REMOVED — kein Evidenz-Rang.** Dieser 19-Modell-Extraktionsbenchmark (Harness v1, N variiert 57–92 je Modell) wurde durch die Within-Subject-Replikation (59 Messungen / 54 Modelle, §5.10) und den seed-fremden Faktortest (§2.9b, Harness v2) ersetzt: Harness v1 ist konfundiert (C23 — CARD/LANG-Faktoren nicht getrennt; mimo-v2.5-pro inzwischen Owner-retired; MiniMax-M3 Owner-excluded). Tabelle und Befunde bleiben zu Dokumentationszwecken erhalten, tragen aber keine Validierungs-Behauptung.

Ursprünglich — um zu überprüfen, ob die LDS-Ergebnisse nicht von einem einzelnen Extraktionsmodell getrieben werden — führten wir einen **19-Modell-Benchmark** über drei API-Plattformen durch (DashScope, DeepSeek API, OpenCode GO). N varies 57–92 per model (failures excluded). Alle Modelle extrahierten Konzepte aus denselben N≥50 Goldstandard-Items:
...
**Archiv-Befunde (historisch, nicht-evidentiell)**: (1) Alle 19 Modelle erreichten F1 > 0,55. (2) Qwen-plus (damaliges primäres Extraktionsmodell) belegte Rang 3 mit F1=0,666. (3) DeepSeek-Modelle waren wettbewerbsfähig. (4) Der enge F1-Bereich (0,55–0,67) über Architekturen (Qwen, DeepSeek, GLM, MinMax, Kimi, Mimo) wurde ursprünglich als Aufgabeneigenschaft gelesen — diese Lesart ist durch C23 (Harness-v1-Konfundierung) und den seed-fremden Sozial-Kollaps (spark 0,10–0,15 vs. DB-Pfad 0,939) überholt; maßgeblich ist §2.9b.

Ein sekundärer Archiv-Befund: 186 zusätzliche DashScope-Modell-Evaluierungen (Text, Vision, Sprache — Evaluierungen, keine Replikations-Modelle) erzeugten alle F1=0,0. GPT-4o und GPT-4o-mini waren aufgrund von Guthabenbeschränkungen während des Benchmarks nicht mehr verfügbar.

### 8.10 Threats to Validity

Wir identifizieren sechs Hauptbedrohungen für die Validität der berichteten Ergebnisse.

**Abhängigkeit vom Extraktionsmodell**. Alle LDS-K-Berechnungen und die Gold-Validierung hängen von der Konzeptextraktion mittels qwen-plus (D1-Produktionsmodell) ab; die N=15-LDS-C-Messungen wurden dagegen mit deepseek-v4-flash extrahiert (s. §8.7-Hinweis). Die Gold-Absicherung trägt nur begrenzt: Batch B (Sozial, n=72) ist **Developing (C9b)** — same-source Seed-Evaluierung, Blind-Review ausstehend — sodass die „Validierung" für die Hauptdomäne vorläufig bleibt. Die modellübergreifende Konsistenz stützt sich auf die Within-Subject-Replikation (59 Messungen / 54 Modelle, §5.10: alle 59 ZH-DE-Paare signifikant; 9/177 englisch-haltige Tests n.s.) und den Harness-v2-Faktortest (§2.9b); der archivierte 19-Modell-Benchmark (§8.9, REMOVED) trägt hierzu nichts bei. Eine andere Extraktionsarchitektur könnte systematisch andere Konzeptmengen erzeugen und die LDS-Werte verändern — diese Bedrohung gilt, gemildert nur auf Topologie-Ebene.
...
ZH-DE nominell 0,519 (frozen v3) — T1-falsifiziert (dekontaminiert 0,990; Size-Matching kehrt Wiki-Vergleich um); nicht als Konvergenz-Evidenz werten (Abb. 10). ZH-EN (0,933) und DE-EN (0,938) liegen im Bereich „nahe am Rauschboden“ — nicht unterscheidbar von zwei zufälligen Hälften desselben sprachlichen Lehrbuchgraphen. Hierbei handelt es sich in erster Linie um Beobachtungen und nicht um Erklärungen; der Mechanismus, der die Heterogenität zwischen den Paaren antreibt, erfordert weitere Untersuchungen.

### 8.12 Einschränkungen

Mehrere Einschränkungen sollten anerkannt werden:

**Umfang der Daten**. Der Mathematik-Korpus (68 Lehrbücher, 556 Konzepte) ist umfassend. Der Physik-Korpus (367 Konzepte) und der Chemie-Korpus (220 Konzepte) ermöglichen eine disziplinübergreifende Validierung, bleiben aber kleiner. Die Lehrplangraphen decken zwar vier Systeme ab (NRW, UK, US, China), verwenden jedoch unterschiedliche Abgleichsmethoden, die die Vergleichbarkeit beeinträchtigen können.

**Extraktionsmethodik**. Während unsere Goldstandard-Validierung eine insgesamt hohe Qualität belegt (F1=0,881 gewichtet gesamt; 0,939† nur Sozial-Subgruppe Batch B, n=72, **Developing**), wurden die sozialwissenschaftlichen Golddaten mittels LLM-Vorextraktion (qwen-plus, temp 0,3) mit anschließender manueller Annahme (`machine-seeded/human-accepted`, `scripts/expand_gold_dataset.py:45-60`) erzeugt. Im Goldstandard selbst könnten einige Fehler verbleiben.
...
Ein zentraler Vorbehalt gegen den Within-Subject-Befund war dessen Beschränkung auf ein einzelnes Modell (deepseek-v4-flash). Eine Replikation des identischen P1-Protokolls (3 Sprachen × k=10, gleicher Messrahmen LDS-C/Floor/Nullmodelle) auf **58 weiteren Modellen** (54 eindeutige Modell-Identitäten; 42 über DashScope, 8 über Bare-Hosts (zen/OpenRouter/D1), 2 über Kilo, je 1 über Cohere/NIM/Cloudflare/LM-Studio, 3 über opencode-go) — DeepSeek-, GLM-, Kimi-, MiniMax- und Qwen-Familien sowie nemotron-3-ultra + nemotron-3-super (NVIDIA, **US-Ursprung**), laguna-s-2,1 (Poolside, **US-Ursprung**, auf zwei Hosts), **gpt-oss-20b (OpenAI-Gewichte, US-Ursprung)**, **command-a (Cohere, CA-Ursprung)**, gpt-5,6-luna (Herkunft ungeklärt, offengelegt), **grok-4,6 (xAI, US-Ursprung)**, **llama-3,3-70b (Meta-Gewichte, US-Ursprung)** und **muse-spark-1.3-contributor (Herkunft offengelegt)** und **phi-4-mini-instruct (Microsoft, US-Ursprung, lokal)** — erweitert die Evidenz:

**◆ Befund 1 — Signale replizieren breit, aber nicht ausnahmslos**: **Alle 59 Messungen zeigen LDS-C über dem jeweiligen Within-Language-Boden, und alle 59 ZH-DE-Paare sind signifikant** (p<0,05 unkorrigiert; kein exaktes p=0,0, s. p-Reportung §5.10/Robustheit (a); Verfügbarkeitsstichprobe; die meisten p<0,01). Von den 177 Sprachpaar-Tests insgesamt sind **9 nicht signifikant** (p≥0,05) — **alle neun betreffen englisch-haltige Paare** (ZH-EN oder DE-EN), überwiegend bei DeepSeek-R1/Distill-Modellen (deepseek-r1-0528 DE-EN p=0,672). Die sprachübergreifende Wertedivergenz ist damit **eine breit replizierbare Eigenschaft mehrsprachiger LLMs**, aber mit modell- und sprachpaar-spezifischer Stärke — insbesondere ist die englisch-bezogene Divergenz in einigen Modellen statistisch nicht von der sprachinternen Varianz trennbar.
...
**◆ Befund 3 — Die Kulturrichtung übersteigt das Zufallsniveau, am stärksten bei hoher Übereinstimmung**: Eine Voten-Analyse der ZH-DE-Divergenztreiber (wie viele Modelle markieren ein Konzept als nur-in-DE bzw. nur-in-ZH) wird gegen ein **frequenz-angepasstes Zufalls-Nullmodell** getestet (beobachtete Gesamthäufigkeit jedes Konzepts fixiert, Richtung jeder Stimme zufällig mit p=0,5). Die beobachtete Zahl richtungskonsistenter Konzepte (max(DE,ZH)-Stimmen ≥ t) übersteigt das Nullmodell bei **allen** Schwellen (unkorrigiert p<0,001 je Schwelle; 3 Schwellen, keine Mehrfachtest-Korrektur; Doppelstimmen-Bias ≤10, s. Robustheit (b); Richtung Preliminary, modell-/paarabhängig), am stärksten bei hoher Übereinstimmung: ≥10 Stimmen 237 vs. 157±4; ≥20 Stimmen 72 vs. 14±2. Die stärksten Konzepte (Heimat:safety 48 Stimmen DE; Heimat:physical space 45 ZH-Stimmen; equal opportunity 45) stützen die DE-Autonomie/ZH-Raum-Orientierung. Wichtig: Die Stärke ist begrenzt — z. B. produzierten nur 48 von 62 Modellen [Datei-Wahrheit inkl. qwen-max-Teilmessung n=26/30; publiziert 59/54] Heimat:safety überhaupt als einseitigen Treiber, und 5 Modelle wurden auf zwei Hosts gemessen (Redundanz).

**▲ Ehrliche Abgrenzung**: Die Replikation umfasst 54 eindeutige Modell-Identitäten; 5 Modelle wurden auf zwei Hosts gemessen (DashScope + Bare-Host ×4, laguna Bare-Host + Kilo), was die Effektivität der "59" verringert; **~81 % der Messungen stammen von chinesischen Anbietern** — der westliche Anteil umfasst elf Messungen (NVIDIA nemotron-3-ultra + nemotron-3-super, Poolside laguna-s-2,1 auf zwei Hosts, OpenAI-Gewichte gpt-oss-20b, Cohere command-a, Meta-Gewichte llama-3,3-70b, xAI grok-4,6, Microsoft phi-4-mini/lokal, muse-spark/Herkunft offengelegt, gpt-5,6-luna/Herkunft ungeklärt). Eine Anbieter-Stratifizierung der 59 vollständigen Messungen (`scripts/sw_fix_analyses.py`): CN-Stratum 48/48 ZH-DE-Paare signifikant (mittlere Marge 0,130), West-Stratum 11/11 signifikant (mittlere Marge 0,155) — richtungskonsistent, aber als Anbieter-*Vergleich* weiter unterpowert, daher Transparenzbericht. Dass Modelle herkunftsunabhängig stärker mit US-Werten alignieren, ist als Branchenmuster dokumentiert [42]; die Herstellererweiterung (Modell-Matrix) bleibt registrierte Folgearbeit. Die Divergenz ist als **Messgröße** (nicht als normatives "Problem") zu interpretieren: Ein Modell, das Wertkonzepte in der Zielsprache kulturadäquat rahmt, ist nicht notwendigerweise fehlerhaft. Als **Vorschlag für einen operativen Schwellenwert** ("hohe sprachübergreifende Divergenz") dient die ZH-DE-Marge (LDS-C − Boden) ≥ 0,10 — bezogen auf die beobachtete Spanne von +0,03 bis +0,42 über 59 Messungen trennt dieser Wert die hoch-divergenten Modelle (command-a, kimi-k2,6, deepseek-v3.x, nemotron) von den niedrig-divergenten (deepseek-v4-flash/pro, r1-Distill). Dies ist eine **heuristische** Festlegung für die Anwendung, keine validierte Validitätsgrenze; eine kalibrierte Schwelle bleibt zukünftiger Arbeit überlassen. Eine Youden-Kalibrierung [46] auf den 59 vollständigen ZH-DE-Margen (Median-Split-Klassen, Bootstrap-CI über Modelle; `scripts/sw_fix_analyses.py`) stützt die Größenordnung: balancierte Trennung bei 0,12 (95 %-CI 0,12–0,13); die Heuristik 0,10 liegt unterhalb des CI und wirkt damit sensitiv-inklusiv — quantifiziert, aber weiterhin keine validierte Grenze. Ehrlichkeitshinweis: Die Klassen stammen aus einem Median-Split derselben 59 Margen, sodass die Trennung (J=1,0) teilweise tautologisch ist; die Kalibrierung ist eine Konsistenzprüfung, keine unabhängige Validierung.

**◆ Robustheit der Replikationsaussage (Sensitivitäten)**: Vier vorausgenommene Einwände wurden quantitativ geprüft. (a) *Multiples Testen*: Bei 177 Tests ohne Korrektur wären unter der globalen Null ~9 falsch-positive Treffer zu erwarten; die Permutationsauflösung (500 Iterationen) kann ein Bonferroni-Niveau (0,05/177≈0,0003) formal nicht auflösen. Entscheidend ist die Meta-Betrachtung: Alle 59 ZH-DE-Paare überschreiten sämtliche 500 Permutationen (p<0,002 einseitig je Test); unter der globalen Null wären 59×0,002≈0,12 solcher Treffer erwartet — beobachtet wurden 59. (b) *Doppelzählung*: Dedupliziert auf 54 eindeutige Identitäten (je Dual-Host eine Messung) bleiben 54/54 ZH-DE-Paare signifikant; mittlere Marge 0,137 (vs. 0,134 über 59), Spanne unverändert +0,03…+0,42. Der Voten-Bias durch Doppelstimmen ist auf ≤10 Stimmen begrenzt und ändert keine Schwellen-Aussage (≥10: 237 vs. 157±4; ≥20: 72 vs. 14±2). (c) *luna-Provenienz*: Ohne das Herkunft-ungeklärte luna-Modell bleiben 10/10 West-Messungen signifikant (mittlere Marge 0,148 vs. 0,155 mit luna). (d) *Selektionsbias*: Von 84 begonnenen Messungen sind 61×n=30 (davon 59 formal: 2 spärliche Kleinstmodell-Läufe ausgeschlossen, s. u.) + qwen-max n=26/30; 23 blieben unvollständig (API-Limits, Kosten-Stopp, Modell-EOL) — der Ausfall korreliert mit westlichen/ratelimitierten Kanälen und ist in `docs/session_handoff_20260910.md` dokumentiert. Die Complete-Case-Analyse bleibt damit eine Verfügbarkeitsstichprobe, kein präregistriertes Design. (e) *Floor-Normierung*: Die Rangordnungs-Aussage (Befund 2) wurde zusätzlich floor-normiert geprüft (Ratio = LDS-C/Boden, je Modell in den Replikationsdaten vorhanden): Die Rangkorrelation zwischen Marge und Ratio beträgt Spearman 0,997, die Top-5 sind identisch, command-a bleibt auch normiert Rang 1 (Ratio 1,87); West-Ratio im Mittel 1,242 vs. CN 1,169. Die Rangordnung wird daher nicht vom Floor-Niveau getrieben — der Kollaps-Hinweis zu command-a (Boden ~0,49) betrifft die absolute Deutung, nicht die Ordnung. (f) *Familie×Sprachpaar*: Die 9 nicht-signifikanten EN-Tests konzentrieren sich auf die R1/Distill-Familie — 6/10 EN-Tests bei 5 R1/Distill-Modellen vs. 3/109 bei allen übrigen (inkl. phi-4-mini ZH-EN p=0,224); der mittlere EN-Boden liegt bei R1/Distill höher (0,844 vs. 0,775). Die Konfundierung Sprache×Modellfamilie ist damit benannt und quantifiziert: Die EN-Hub-Deutung bleibt Hypothese, die Alternative „Reasoning-Verbosität hebt den Boden und schluckt das EN-Signal" ist nicht ausgeschlossen.
**◆ Kleinstmodell-Grenze (local wave, nicht gezählt)**: Dasselbe P1-Protokoll lief auf vier lokalen Kleinstmodellen (≤3,8B). Nur **phi-4-mini-instruct** (Microsoft, 3,8B) besteht die formale Hürde (ZH-DE +0,087, p<0,01; ~140 Konzepte/Sprache vs ~295 Baseline) und ist als 59. Messung promoviert — das Sprachsignal überlebt bei 3,8B lokal. Die anderen drei definieren die **untere Auflösungsgrenze des Protokolls**: **qwen2.5-0.5b-instruct** (Alibaba, 0,5B) ist ZH-DE null (p=0,168) bei spärlicher Extraktion (zh 36 Konzepte gesamt; Boden NaN) — ehrliche Null, kein Beleg gegen das Signal; **hy-mt2-1.8b** (Tencent, Übersetzungs-Spezialist) liefert ganze leere Topic-Sets (zh Freiheit/Heimat/Erfolg = 0 Konzepte) → Boden NaN überall, Marge undefiniert, sein ZH-EN LDS=1,0 ist ein Leermengen-Artefakt (retired Kategorie); **gemma-3-270m-it** (Google, 0,27B) folgt dem JSON-Extraktionsschema nicht (String-Konzepte ohne en-gloss, 4/5 Topics) → Quarantäne, Re-Run ausstehend. Die Kette bricht bei der Extraktion, nicht beim Antworten (Dichte-Schwelle ≈ 50 Konzepte/Sprache). Registrierte Folgearbeit: Kleinstmodell-Extraktionsschiene.
