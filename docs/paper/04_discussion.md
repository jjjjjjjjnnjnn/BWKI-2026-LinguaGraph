...
Ein sekundärer Befund: 186 zusätzliche DashScope-Modell-Evaluierungen (Text, Vision, Sprache — Evaluierungen, keine Replikations-Modelle) erzeugten alle F1=0,0, was bestätigt, dass diese andere Prompting-Strategien erfordern. GPT-4o und GPT-4o-mini waren aufgrund von Guthabenbeschränkungen während des Benchmarks nicht mehr verfügbar.

### 8.10 Threats to Validity

Wir identifizieren sechs Hauptbedrohungen für die Validität der berichteten Ergebnisse.

**Abhängigkeit vom Extraktionsmodell**. Alle LDS-K-Berechnungen und die Gold-Validierung hängen von der Konzeptextraktion mittels qwen-plus (D1-Produktionsmodell) ab; die N=15-LDS-C-Messungen wurden dagegen mit deepseek-v4-flash extrahiert (s. §8.7-Hinweis). Die Gold-Absicherung trägt nur begrenzt: Batch B (Sozial, n=72) ist **Developing (C9b)** — same-source Seed-Evaluierung, Blind-Review ausstehend — sodass die „Validierung" für die Hauptdomäne vorläufig bleibt. Während der 19-Modell-Benchmark die modellübergreifende Konsistenz bestätigt (F1-Bereich 0,55–0,67; N variiert 57–92 je Modell, Failures exkludiert), könnte eine andere Extraktionsarchitektur systematisch andere Konzeptmengen erzeugen und möglicherweise die LDS-Werte verändern. Diese Bedrohung wird durch den engen F1-Bereich über verschiedene Modellfamilien hinweg teilweise abgemildert.
