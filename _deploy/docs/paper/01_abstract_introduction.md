...
- Erster systematischer Vergleich mathematischer Wissensstrukturen über ZH/EN/DE hinweg (556 Konzepte, Nullmodell-Suite, Within-Subject-Replikation mit 59 Messungen / 54 Modellen)
...
- **LLM-as-Subject (Within-Subject)**: Dasselbe LLM antwortet in ZH/DE/EN auf dieselben 5 Themen → **Sprachsignal ist nachweisbar** (LDS-C 0,93–0,96 ≫ Boden 0,85–0,87); LMM identifiziert den **Sprachcode als dominanten Organisator** (same_lang +0,038, p<0,001; same_frame +0,001, p=0,90); ZH-DE trägt eine strukturelle Ebene jenseits der Assoziationsstatistik; eine **modellübergreifende Replikation** (54 eindeutige Modelle, überwiegend chinesische Anbieter plus elf westliche Messungen) zeigt: alle 59 ZH-DE-Sprachpaare signifikant (p<0,05; ohne Mehrfachtest-Korrektur, Verfügbarkeitsstichprobe), aber 9/177 Sprachpaaren (alle englisch-bezogen) nicht; die Kulturrichtung (DE Autonomie/Regeln vs. ZH Raum/Anspruch) übersteigt ein frequenz-angepasstes Zufalls-Nullmodell bei allen Schwellen (p<0,001), am stärksten bei hoher Übereinstimmung (237 Konzepte mit ≥10 Stimmen vs. 157 zufällig erwartet)
- **Anwendungsfall — KI-Validierung (heuristisch, kein fertiges Audit-Instrument)**: Divergenzbericht pro Modell und Sprachpaar (LDS-C + konkret divergierende Konzeptbestandteile + Domänen-Asymmetrie) — für Entwickler (Vorabprüfung mehrsprachiger Modelle), Regulierer (Transparenz-Heuristik, kein EU-AI-Act-Compliance-Beleg) und Forscher (kulturelle Werte in KI)

Die Arbeit demonstriert, dass LLM-gestützte Graphanalyse ein vielversprechendes Werkzeug zur Untersuchung sprachlicher Einflüsse auf die Wissensorganisation darstellt — mit Implikationen für die bilinguale Bildung und die KI-Forschung. Eine Null Model Suite falsifiziert die Annahme sprachgetriebener Lehrbuchdivergenz und etabliert ΔLDS als Kernmetrik. Die erweiterte Humanvalidierung (N=15) falsifiziert ΔLDS > 0 unter Between-Subject-Bedingungen; ein LLM-as-Subject-Experiment (Within-Subject) weist das Sprachsignal dagegen klar nach und stützt die Design-Artefakt-Hypothese für den Human-Negativebefund (Exklusion + Konsistenz-Demonstration; keine quantitative Kausalzuordnung) — mit dem Sprachcode als dominantem Organisationsebene-Kandidaten und dem kulturellen Rahmen als sekundärem, code-internem Modulator.
...
Die Arbeit ist wie folgt aufgebaut. Kapitel 2 ist zweiteilig angelegt: 2A gibt einen Überblick über verwandte Arbeiten aus Linguistik, KI-Forschung und Wissensgraphen, 2B beschreibt die Methodik beider Pipelines (Textbuchgraph + LLM-as-Subject). Kapitel 3 präsentiert die Ergebnisse — vom CognitiveSpace-Wissensgraphen über die LDS-Analyse bis zur Humanvalidierung (F1–F4, F6–F12; F5 als entfernte Vorstudie nicht-evidentiell). Kapitel 5 weist das Sprachsignal im LLM-as-Subject-Within-Subject-Design nach, Kapitel 6 validiert fächerübergreifend (Physik, Chemie), Kapitel 7 exploriert die Sprachproduktion (LPA, Pilot). Kapitel 8 diskutiert Ergebnisse, Limitationen und die methodologische Einordnung; Kapitel 9 fasst zusammen und formuliert die Audit-Anwendung (heuristisch).
