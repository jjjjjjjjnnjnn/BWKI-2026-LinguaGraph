...
| `math_extractions/` | 69 JSON files | 📦 Referenced only | MIMO LLM extraction outputs — concept/relation JSON from each textbook section |
| `questionnaires/` | 9 files | ✅ Tracked | BWKI survey instruments in ZH/EN/DE |
| `gold/` | 2 JSON | ✅ Tracked | Gold-standard human annotations |
| `baseline/` | 1 JSON | ✅ Tracked | Computational baseline results |
| `corpus/` | 30 text files | ✅ Tracked | Wikipedia pilot corpus (ZH/EN/DE, 5 topics) |
| `evidence/` | 7 files | ✅ Tracked | Research evidence and analysis summaries |
| `curricula/` | 22 files | ✅ Tracked | Curriculum standards (NRW/UK/US/CN, git ls-files) |
| `lds_c/` | 119 files | ✅ Tracked | LDS-C human + LLM-as-subject results (git ls-files) |
| `model_comparison/` | 19 files | ✅ Tracked | Model benchmark outputs (git ls-files) |
| `quality_history/` | 11 files | ✅ Tracked | Quality/QC history (git ls-files) |
| `raw/` | 3 files | ✅ Tracked | Raw staging (git ls-files) |
| `wikipedia_extractions/` | 17 files | ✅ Tracked | Wikipedia extraction outputs (git ls-files) |
| `consent/` | 2 files | ✅ Tracked | Consent/ethics records (git ls-files) |
...
- `题名:普通高中教科书·物理/化学7册 (PEP 2019) | 作者:人教社课材所 | 版次:2019(第1版) | 出版社:人民教育出版社 | 年份:2019 | 章节:物理必修3(143pp)/选必1(130pp)/选必2(122pp)/选必3(143pp)/化学选必1(139pp)/选必2(115pp)/选必3(163pp) | 语言:zh | 许可+URL:版权教材·第三方镜像(TapXWorld/ChinaTextbook, provenance unverified)·本地研究专用·永不进仓·以smartedu官方本为准 | 到货:2026-09-12 | 格式:扫描版无文本层 | 封面已目验(2019审定章+人教社印) | OCR:2026-09-12完成 RapidOCR DirectML 150dpi 事实级(31章txt~1.55MB+逐页txt,均值置信度0.95-0.985,9份QC报告在Temp/opencode/qc_*.md) | 实测修正:化学选必1无§1.3,化学反应的方向为§2.3(映射已修) | MIT bundle附带Ch27-29(流体/动理论)一并抽取(29章txt) | R闭环接地:2026-09-12 `scripts/ground_refs.py`+`config/expert_graphs/text_grounding_20260912.json`(545KB):物理367节点ZH接地66.5%/EN33.8%,化学220节点ZH65.9%/EN25.5%;未接地主因=大学节点(高中书天然无)+DE无文本(LEIFI link-only);EN短语召回偏低如实记 | 必修1/2四册补齐(物理必修1/134pp/6章txt·必修2/114pp/4章·化学必修1/139pp/6章·必修2/139pp/4章,均值0.95-0.98),映射扩至11册(7 mapped+4 stubs,节映射pending) | EN语义接地:2026-09-12 `scripts/semantic_ground_en.py`(nomic-embed预筛)+`scripts/sem_shards.py`(10分片)+10路Spark并行裁决(substantive-description规则,passing-mention拒收)→`config/expert_graphs/text_grounding_en_semantic_20260912.json`(107KB,165/407命中40.5%):物理EN 33.8%→62.1%,化学EN 25.5%→53.2%;层分离记账(不与substring混数),温度0复现链完整 | 反向注:EN层 substring 33.8/25.5 vs semantic 62.1/53.2(portal src_local已同步)`
