...
- **修复**: cell 级随机效应（5 cell×5 topic）或 cluster-robust SE；Hessian 正定检查；df 用 Kenward-Roger 近似并明示小样本局限。**论文 §5.6/§8.14 引用 same_lang p<0.001 时需注明此推断局限**。
...
### 深入 A（✅ 已完成 2026-08-09）：异质性注入——设计伪影的"直接因果"证明
- **结果**（`scripts/lds_c_heterogeneity_injection.py` → `heterogeneity_injection_20260809.json`）：
  - 基线：人类与 LLM 的 within-language 两两重叠几乎相同（ZH 0.057 vs 0.059；DE 0.106 vs 0.108）——单样本异质性可比；floor 差异源于**聚合稀疏度**。
  - 注入：向 LLM 样本做概念 dropout（q=1→0.3），**信号余量单调坍缩**：+0.071 → +0.014（ZH-DE）。
  - **校准点**：q=0.30 时 LLM floor 0.941 ≥ 人类 floor 0.921，余量 +0.014 ≈ 人类 +0.015——**精确复刻人类阴性**。
  - **结论**：人类阴性被**直接因果地**归因于组内异质性（聚合稀疏度），而非"无语言效应"——从排除法升级为正面因果证明。
  - 已写入论文 §5.9.2b + 讨论 §8.14 第 4 点。
